...
class GildedRoseSpec extends Specification {
    ...
    def "Once the sell by date has passed, Quality degrades twice as fast"() {
        ...
        List<Item> items = [new Item("Basketball", 0, 10)]
        ...
        app.items.get(0).name == "Basketball"
        app.items.get(0).quality == 8
    }


    def "The Quality of an item is never negative"() {
        given: "Basketball has no quality anymore"
        List<Item> items = [new Item("Basketball", 5, 0)]
        GildedRose app = new GildedRose(items);
    ...
    def "The Quality of an item is never more than 50"() {
        ...
        then: "the quality remains zero"
        app.items.get(0).name == "Aged Brie"
        app.items.get(0).quality == 50
    }
    ...
    def "Backstage passes increases in Quality the older it gets"() {
        given:
        List<Item> items = [new Item("Backstage passes to a TAFKAL80ETC concert", 12, 40)]
        GildedRose app = new GildedRose(items)
        ...
        app.items.get(0).name == "Backstage passes to a TAFKAL80ETC concert"
        app.items.get(0).quality == 41
    }


    def "Concert Tickets have correct quality over time" () {
        given:
        List<Item> items = [new Item("Backstage passes to a TAFKAL80ETC concert", 12, 30)]
        GildedRose app = new GildedRose(items)

        when: "quality is updated over time"
        app.updateQuality()
        def qualityWhen11DaysAreLeft = app.items.get(0).quality
        updateNrOfDays(app, 4)
        def qualityWhen7DaysAreLeft = app.items.get(0).quality
        updateNrOfDays(app, 3)
        def qualityWhen4DaysAreLeft = app.items.get(0).quality
        updateNrOfDays(app, 5)
        def qualityAfterTheConcert = app.items.get(0).quality
